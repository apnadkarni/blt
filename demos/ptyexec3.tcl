...
set command [list /bin/sh -c "$program scripts/bgtest.tcl"]
set animate(index) -1
...
proc InsertText { string tag } {
    .text insert end "[clock format [clock seconds]]: " date
...
# Create widgets
text .text 
.text tag configure stdout -font { Arial 9 bold } -foreground green2
.text tag configure stderr -font  { Arial 9 } -foreground red2
.text tag configure date -font { Arial 6 } -foreground blue4
