...
proc FormatPicture { w row col } {
    global images 
    set table [$w cget -table]
    set file [$table get $row name ""]
    if { $file == "" } {
    ...
    if { [file isdirectory ./images/$file] } { 
	return "y"
    }
    if { [info exists images($file)] } {
	return $images($file)
    }
    if { [catch {image create picture -file ./images/$file} img] != 0 } {
	return $file
    }
    set w [image width $img]
    set h [image height $img]
    if { $w > 50 || $h > 50 } {
	set small [image create picture -width 50 -height 50]
	$small resample $img -maxpect
	image delete $img
	set images($file) $small
	return $small
    }
    set images($file) $img
    return $img
...
$view column configure button \
    -style button \
    -title "Picture" \
    -formatcommand [list FormatPicture $view]
